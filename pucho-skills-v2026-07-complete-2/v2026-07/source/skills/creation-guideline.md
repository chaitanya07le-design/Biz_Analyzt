...
Author the docs the tier requires, in order. **Save the confirmed docs into the project repo as `/docs/01-prd.md … 06-plan.md`** (or the ClickUp task description for non-code engagements) so every downstream agent/skill reads the same source of truth instead of re-deriving. Keep them tight and concrete — these are the build's source of truth, not prose essays. Templates:
...
**Platform availability:** on Claude, all four routes exist (`pucho-frontend` supersedes `pucho-dashboard-template` for product UI — if only the old skill is installed, apply pucho-frontend's tokens over it). On OpenCode, `pucho-blueprint-doc` is unavailable — produce a structured markdown deliverable and note that the branded .docx is generated on the Claude side.

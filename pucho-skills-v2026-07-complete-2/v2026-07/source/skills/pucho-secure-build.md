...
Any prompt this skill emits names these explicitly so the build model doesn't substitute Express/Firebase/NextAuth. Weaker models substitute stacks more readily — repeat the stack in the prompt's opening AND closing.
...
### 2 — Rate limiting
```
SECURITY — RATE LIMITING: Rate-limit every Supabase Edge Function. Auth actions (login, register, password reset): 5 req / 15 min per IP. General data routes: 60 req / min per IP. Pucho webhook-proxy / LLM routes: 10 req / min per authenticated user. File uploads: 5 req / min per IP. Return HTTP 429 with a Retry-After header when exceeded. Implement with a Supabase-table counter (or Upstash Redis) keyed by IP/user — Edge Functions are STATELESS, so an in-memory counter resets per instance and is best-effort only, never the real limit.
```
...
### 5 — Auth hardening
```
SECURITY — AUTH: Use Supabase Auth (do not hand-roll). Passwords hashed by Supabase (bcrypt/argon2) — never store plaintext. Access tokens expire in 15–60 min. Session storage: prefer httpOnly-cookie handling via server-side auth (Edge Function / @supabase/ssr); if using the plain supabase-js browser client (Vite SPA default), accept its managed storage but compensate with a strict CSP and no third-party scripts — NEVER hand-roll your own token persistence. JWT secret ≥32 chars from env. Enable account lockout / exponential backoff after repeated failed logins.
```
...
## INTEGRATION
- **With `pucho-frontend`** (or the deprecated `pucho-dashboard-template`): after generating the UI, append §1–5, 7, 9, 10, 11 to the build prompt. The dashboard's webhook action buttons MUST call a Supabase Edge Function proxy, not the Pucho URL directly — this skill overrides any direct-call pattern.
- **With build-prompt generation (any agentic IDE):** add a `## SECURITY REQUIREMENTS (non-negotiable)` section to the prompt, populated from the scope table for that deliverable type, before the final output.
...
- [ ] Lockfile committed; `npm audit` clean of criticals; no unused deps
- [ ] RLS verified by TEST: second user account cannot read/write first user's rows

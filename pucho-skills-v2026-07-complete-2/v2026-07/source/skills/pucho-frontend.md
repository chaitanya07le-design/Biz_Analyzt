...
## BUILD PIPELINE (MANDATORY — in order, every build)
1. New project/requirement? → run `creation-guideline` first; build against its confirmed App Flow + UI/UX brief.
2. Define `src/config/nav.js` FIRST (§3) — every page and the header title derive from it.
3. Clone canonical pieces, don't restyle: Login (`reference/login/Login.jsx` byte-for-byte), `Input.jsx`, badge (§7), components (§4).
4. Wire data/webhooks ONLY through `triggerWorkflow()` → Edge Function proxy (§8); mock-first with the MOCK pill (§4B).
5. Walk the §10 checklist line by line before presenting; fix every miss.
...
> **Two Pucho palettes exist by design — do not "correct" one to the other.** PRODUCT UI (this skill + all app screens): violet `#8b5cf6` family, Inter/Space Grotesk. DOCUMENTS & BRAND COLLATERAL (blueprints, PPTX, MoMs — `pucho-brand-guidelines`): Pucho Violet `#5922C6`, Anek Latin/Lato/Chivo Mono. A dashboard is never Anek `#5922C6`; a client .docx is never Inter `#8b5cf6`.
...
> Known quirk: the icon filenames contain spaces/`=`/`,` (Figma export names). Vite handles them when imported with quoted paths exactly as Login.jsx does; do NOT rename the files (renaming breaks the byte-for-byte Login).
...
import { supabase } from '../utils/supabase'; // createClient(VITE_SUPABASE_URL, VITE_SUPABASE_ANON_KEY)
const AuthCtx = createContext(null);
export const useAuth = () => useContext(AuthCtx);
export function AuthProvider({ children }) {
  const [user, setUser] = useState(null);
  const login = async (email, password) => {
    // Supabase Auth directly (per secure-build §5 — never hand-roll an auth endpoint).
    const { data, error } = await supabase.auth.signInWithPassword({ email, password });
    if (error) return { success: false, message: 'Invalid credentials' }; // generic — never echo error.message
    setUser(data.user);
    return { success: true };
  };
...
If the CDN logo URL is unreachable (offline/demo builds), fall back to the local asset `reference/login/assets/brand/logo.png` — same size classes, same everything else.

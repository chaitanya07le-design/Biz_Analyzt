...
const TopBrands = () => {
  ...
  const brandData = useMemo(() => {
    ...
    if (vouchers && voucherLines) {
      const salesVoucherIds = new Set(
        vouchers.filter(v => v.VoucherType === 'Sales').map(v => v.VoucherID)
      );
      const purchaseVoucherIds = new Set(
        vouchers.filter(v => v.VoucherType === 'Purchase').map(v => v.VoucherID)
      );
  ...
  const sortedBrands = useMemo(() => {
    ...
    switch (sortBy) {
      case 'sales':
        return sorted.sort((a, b) => b.salesValue - a.salesValue);
      case 'items':
      ...
      case 'margin':
        return sorted.sort((a, b) => {
          const marginA = a.salesValue > 0 ? ((a.salesValue - a.purchaseValue) / a.salesValue) * 100 : 0;
          const marginB = b.salesValue > 0 ? ((b.salesValue - b.purchaseValue) / b.salesValue) * 100 : 0;
      ...
      <div className="px-4 py-4 md:px-6 md:py-6">
        ...
        >
          ...
          <p className="text-sm text-ink-muted mt-1">
            Brand-wise sales performance and inventory analysis
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-ink-muted">Brands</span>
            </div>
            <div className="text-lg font-bold text-ink-default">
              {brandData.length}
            </div>
          ...
          >
            ...
            <div className="text-lg font-bold text-ink-default">
              {formatCurrency(totalSales / Math.max(brandData.length, 1))}
            </div>
          </motion.div>
        </div>
        ...
        >
          ...
          <div className="divide-y divide-canvas-faint">
            {topBrands.map((brand, idx) => {
              const margin = brand.salesValue > 0 
                ? ((brand.salesValue - brand.purchaseValue) / brand.salesValue) * 100 
                : 0;
              const salesPercent = totalSales > 0 
                ? (brand.salesValue / totalSales) * 100 
                : 0;
              ...
              return (
                <motion.div
                  ...
                  onClick={() => navigate(`/masters/items?brand=${encodeURIComponent(brand.brand)}`)}
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm ${
                      ...
                      'bg-canvas-faint text-ink-muted'
                    }`}>
                      #{idx + 1}
                    </div>

...
export default function CreditNoteVouchers() {
  ...
  const cnVouchers = useMemo(() => {
    if (tallyVouchers && tallyVouchers.length > 0) {
      return tallyVouchers.filter((v) => (v.type || '').toLowerCase() === 'credit note').map((v) => {
        ...
        return {
          ...
          VoucherDate: v.date || v.VoucherDate, VoucherType: 'Credit Note',
          PartyName: v.party || v.PartyName, NetAmount: v.amount, Status: v.status || 'POSTED',
          _cnDetails: { taxReversal, reason: v.narration || '', originalInvoiceRef: v.referenceNo || '', lineCount: lineItems.length, lineItems },
        };
      });
    }
  ...
  return <VoucherReportLayout title="Credit Note Vouchers" data={cnVouchers} loading={loading} columns={columns} tallyData={tallyVouchers && tallyVouchers.length > 0 ? cnVouchers : null} isTallyPage={tallyVouchers && tallyVouchers.length > 0} partyList={partyList} onRowClick={(row) => navigate(`/voucher/${row.VoucherID || row.id}`, { state: row._cnDetails || null })} />;
}
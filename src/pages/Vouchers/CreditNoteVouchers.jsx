...
export default function CreditNoteVouchers() {
  ...
  const creditNoteVouchers = useMemo(() => {
    return (vouchers || []).filter(v => v.VoucherType === 'Credit Note' || v.voucherType === 'Credit Note');
  }, [vouchers]);

  const filtered = creditNoteVouchers.filter(v => {
    ...
    <div className="space-y-6">
      <Card>
              ...
              {filtered.map((voucher) => {
                ...
                return (
                  <tr key={voucher.VoucherID || voucher.id} className="hover:bg-ink-50 cursor-pointer" onClick={() => navigate(`/voucher/${voucher.VoucherID || voucher.id}`)}>
                    ...
                    <td className="px-5 py-3 text-right font-medium text-green-600">{formatCurrency(amount)}</td>
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-line">
          <span className="text-xs text-ink-600">Showing {filtered.length} of {creditNoteVouchers.length} vouchers</span>
        </div>
      </Card>
    </div>
  );

...
export default function SalesVouchers() {
  ...
  const salesVouchers = useMemo(() => {
    return (vouchers || []).filter(v => v.VoucherType === 'Sales' || v.type === 'Sales');
  }, [vouchers]);
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedBrand || selectedSubCategory) {
      const salesVoucherIds = new Set(
        salesVouchers.map(v => v.VoucherID)
      );
    ...
    return result;
  }, [salesVouchers, search, selectedBrand, selectedSubCategory, voucherLines, items]);
    ...
    <div className="space-y-6">
      <Card>
        ...
        {showFilters && (
          <div className="mb-6 p-4 bg-ink-50 rounded-xl border border-line">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-medium text-ink-900">Filter Sales</h3>
              {hasFilters && (
                <button 
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-line">
          <span className="text-xs text-ink-600">Showing {filtered.length} of {salesVouchers.length} vouchers</span>

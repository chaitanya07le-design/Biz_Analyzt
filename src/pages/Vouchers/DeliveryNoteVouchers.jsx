...
export default function DeliveryNoteVouchers() {
  ...
  const deliveryNoteVouchers = useMemo(() => {
    return (vouchers || []).filter(v => v.VoucherType === 'Delivery Note' || v.voucherType === 'Delivery Note');
  }, [vouchers]);

  const filtered = deliveryNoteVouchers.filter(v => {
      ...
      <Card>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex gap-2">
            ...
            <Button icon={Plus}>New Delivery Note</Button>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-line">
          <span className="text-xs text-ink-600">Showing {filtered.length} of {deliveryNoteVouchers.length} vouchers</span>
        </div>
      </Card>
    </div>
  );

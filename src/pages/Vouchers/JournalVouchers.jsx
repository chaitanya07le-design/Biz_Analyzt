...
export default function JournalVouchers() {
  ...
  const journalVouchers = useMemo(() => {
    return (vouchers || []).filter(v => v.VoucherType === 'Journal' || v.voucherType === 'Journal');
  }, [vouchers]);

  const filtered = journalVouchers.filter(v => {
      ...
      <Card>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex gap-2">
            ...
            <Button icon={Plus}>New Journal</Button>
          </div>
        </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-line">
              {filtered.map((voucher) => {
                ...
                const particulars = voucher.Narration || voucher.PartyName || voucher.party || '';
                const amount = parseFloat(voucher.NetAmount || voucher.amount || voucher.GrandTotal) || 0;
                const status = voucher.Status || voucher.status || 'POSTED';
                ...
                return (
                  <tr key={voucher.VoucherID || voucher.id} className="hover:bg-ink-50 cursor-pointer" onClick={() => navigate(`/voucher/${voucher.VoucherID || voucher.id}`)}>
                    ...
                    <td className="px-5 py-3 text-ink-900">{particulars}</td>
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-line">
          <span className="text-xs text-ink-600">Showing {filtered.length} of {journalVouchers.length} vouchers</span>
        </div>
      </Card>
    </div>
  );
